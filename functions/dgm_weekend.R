...
dgm_weekend<- function(sample_size, total_T, ft, beta_1, beta_2, gt, alpha,
  ...
  # gamma2 controls the magnitude of weekend effect 
...
dgm_weekend <- function(sample_size, total_T, ft, beta_1, beta_2, gt, alpha, tau, pt, gamma2, mean_var = 1, j_t = NULL, h_At = NULL) {
  # variable A represent the treatment level  (0 , 1 , 2)
  # variable A1 and A2 is indicator function whether A is equal 1 or 2 respectively
  # variable I is a binary indicating availability of the participant at timepoint t 
  # prob_t indicates the probability of someone available at time point t, assume constant for now
  ...
  df_names <- c("userid", "time", "Y", "A", "A1", "A2", "I", "prob_A","prob_I", "eps", "weekend", "Y_prob1_noEffect", "Y_prob1")
  ...
  for (t in 1:total_T) {
    ...
    dta$Y_prob1_noEffect[row_index] <- exp(dta$A1[row_index] * as.numeric(ft[t, ] %*% beta_1)  +
                                    dta$A2[row_index] * as.numeric(ft[t, ] %*% beta_2) +
                                    as.numeric(gt[t, ] %*% alpha))
...
# beta2_t <- solve_beta(shape = "linear_theta", m=m, ATE = 0.16, ft = ft_t, taut = taut_t, theta = -0.3,gt = gt_t, alpha = alpha_t)
