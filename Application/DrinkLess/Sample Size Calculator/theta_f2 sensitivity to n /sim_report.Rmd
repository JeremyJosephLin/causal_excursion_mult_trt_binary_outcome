...
```{r, eval=FALSE}
# only run this chunk to read new result from HPC
n_seed = 1
...
saveRDS(SD, "n_vs_mee_pattern.RDS")
...
# 1. Make plots for n versus ATE------------------------------------------------
    n_ATE <- readRDS(here("dataset", "n_vs_mee_pattern.RDS"))
    
   n_ATE$ft_t_shape_and_ft_t_theta <- paste0(n_ATE$ft_t_shape, n_ATE$ft_t_theta)
  unique(n_ATE$ft_t_shape_and_ft_t_theta)

   # reorder levels 
    #n_ATE$ft_t_shape_and_ft_t_theta <- factor( n_ATE$ft_t_shape_and_ft_t_theta, levels = c("constant0","linear_theta-0.3",   "linear_theta0.3"))
   
    n_constant_MEE <- n_ATE$n[n_ATE$ft_t_shape == "constant"][1]
   
    # ggplot 
    n_ATE %>% 
    filter(ft_t_shape != "constant") %>% 
    ggplot(aes(x = ft_t_theta2, y = n, color = ft_t_shape_and_ft_t_theta)) +
    geom_line() +
    ...
    ggsave("size Drinkless n vs ft2_shape.jpg", height = 6, width = 6)
    ggsave("size Drinkless n vs ft2_shape.pdf", height = 6, width = 6)
```
