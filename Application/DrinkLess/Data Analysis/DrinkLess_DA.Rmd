...
knitr::opts_chunk$set(echo = FALSE, message = FALSE, 
                      ...
                      eval = TRUE)
# load data set 
source(here("functions", "emee_catA.R"))
source(here("functions", "functions_util.R"))
```
...
```{r eval=TRUE}
dta <- readRDS(here("dataset","DrinkLess_processed.RDS"))
...
```{r}
fit_1a_summary <- emee_catA_summary(fit1a)
...
diff1a <- emee_catA_glh(fit1a, L1a)
...
```{r}
fit_1b_summary <- emee_catA_summary(fit1b)
...
diff1b <- emee_catA_glh(fit1b, L1b)
...
fit_1c_summary <- emee_catA_summary(fit1c)
...
```{r}
fit_1d_summary <- emee_catA_summary(fit1d)
...
# test each effect 
for (effect in 1:7) {
  effect_temp <- emee_catA_glh(fit1d, matrix(diff_matrix_1d[effect, ], ncol = 6))
