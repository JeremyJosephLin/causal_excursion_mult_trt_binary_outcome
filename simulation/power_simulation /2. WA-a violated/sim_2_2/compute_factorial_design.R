...
SD <- expand.grid(gamma = 0.05,  # type I error
                  ...
                  q_w = NA,
                  n = NA,
                  ATE_tw_ratio = seq(from = .5, to = 1.5, by= 0.1),
                  p_star = NA,
                  error_indicator = 0,
...
saveRDS(SD, file = "~/Documents/Research/causal-excursion-multi-treatment-binary-outcome/simulations/power_simulation /2. WA-a violated/sim_2_2/sim_2_2b/factorial_design.RDS")

dim(SD)

#debug purposes 
# see which setting is giving error message  
# dim(SD)
# SD$error_indicator
# which(SD$error_indicator ==1)
# SD[70:90,]
# SD[which(SD$error_indicator ==1),]
# SD[which(SD$error_indicator ==0),"ATE_tw_ratio"]
# SD[which(SD$error_indicator ==0),"ATE1_w"]
# SD[which(SD$error_indicator ==0),"ATE2_w"]
# SD[which(SD$error_indicator ==0),"ATE2_t"]/ SD[which(SD$error_indicator ==0),"ATE1_t"]
# SD[which(SD$error_indicator ==0),"ASPNC_t"]
