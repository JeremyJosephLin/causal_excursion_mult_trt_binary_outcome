# This code generalize the simulation for sim_4_7 weekend effect
...
source("~/Documents/Research/causal-excursion-multi-treatment-binary-outcome/simulations/power_simulation /3. WA-b violated/sim_4_7/sim_4_7b/dgm_weekend.R")
...
SD <- expand.grid(gamma = 0.05,  # type I error
                  ...
                  p_star = NA,
                  error_indicator = 0,
                  gamma2 = seq(from = -.3, to = .3, by = 0.01),
...
for (i in 1:nrow(SD)) {
  ...
  SD$ASPNC_t_empirical[i] <- compute_ASPNC_Wknd(gt= gt_t, alpha = alpha_t, tau = taut_t, gamma2 = gamma2)
  ...
  SD$ASPNC_w[i] <- SD$ASPNC_t_empirical[i]
  ...
  tryCatch({
    ...
    varcov <- compute_sigma(f_t = ft_w, g_t=gt_w, alpha = alpha_w, beta = beta_w, p_t = pt, tau = taut_w)
    M_matrix <- varcov$M_matrix
    Sigma_matrix <- varcov$Sigma_matrix
    
    # Theoretical variance-covariance matrix for beta hat
    var_beta <- 1/SD$n[i] *  solve(M_matrix)%*% Sigma_matrix %*% t(solve(M_matrix))
    SD$se_beta1_w0[i] <- sqrt(var_beta[1,1]) 
    SD$se_beta2_w0[i] <- sqrt(var_beta[2,2]) 
    SD$p_star[i] <- dim(L)[1]
  },
  error = function(cond){
...
saveRDS(SD, file = "~/Documents/Research/causal-excursion-multi-treatment-binary-outcome/simulations/power_simulation /3. WA-b violated/sim_4_7/sim_4_7b/factorial_design.RDS")

# dim(SD)
# SD[which(SD$error_indicator == 1),]
# SD$n
