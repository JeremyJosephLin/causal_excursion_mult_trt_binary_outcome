# This code generalize the simulation for endogeneity misspecification 
# Availability depends on previous treatment and previous outcome
# dgm will be able to 
rm(list = ls())
...
SD <- expand.grid(gamma = 0.05,  # type I error
                  ...
                  n = NA,
                  p_star = NA,
                  gamma3 = seq(from = -0.2, by = 0.02, to = 0.2), # coefficent in dgm
                  gamma4 = seq(from = -0.2, by = 0.02, to = 0.2), # coefficent in dgm
                  gamma5 = seq(from = -0.2, by = 0.02, to = 0.2), # coefficent in dgm
                  error_indicator = 0, 
...
for (i in 1:nrow(SD)) {
  ...
  ## taut_w
  SD$AvgTau_w[i] <- SD$AvgTau_t[i]
  taut_w <- rep(SD$AvgTau_w[i], m) #assume availability is constant for all time point
...
saveRDS(SD, file = "~/Documents/Research/causal-excursion-multi-treatment-binary-outcome/simulations/power_simulation /6.WA-e violated/sim_7_2a/factorial_design.RDS")
