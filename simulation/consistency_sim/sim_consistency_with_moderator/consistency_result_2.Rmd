...
for(j in 1:length(result_list)) {
  ...
  coef_est[j, 3:6] <- rowMeans(coef_method, na.rm = TRUE) - param
  coef_est[j, 7:10] <- rowMeans(th_se, na.rm = TRUE)
  coef_est[j, 11:14] <- apply(coef_method, 1, function(x) sd(x, na.rm = TRUE))
  coef_est[j, 23:26] <- rowMeans(th_se_adj, na.rm = TRUE)
  ...
  coef_est[j, 35:38] <- rowMeans(ci_unadj_lb, na.rm = TRUE)
  coef_est[j, 39:42] <- rowMeans(ci_unadj_ub, na.rm = TRUE)
  
  # ci z-adjusted lowerbound and upperbound
  coef_est[j, 43:46] <- rowMeans(ci_adj_lb_z, na.rm = TRUE)
  coef_est[j, 47:50] <- rowMeans(ci_adj_ub_z, na.rm = TRUE)
  
  # ci t-adjusted lowerbound and upperbound
  coef_est[j, 51:54] <- rowMeans(ci_adj_lb_t, na.rm = TRUE)
  coef_est[j, 55:58] <- rowMeans(ci_adj_ub_t, na.rm = TRUE)
  
  # true paramete 
  coef_est[j ,62:65] <- data$true_beta
  ...
  # collect values of all betas for each setting
  betas <- c(betas, list(list(sample_size = n, 
                                            total_T = t,
                                            beta1 = coef_method[1,],
                                            beta2 = coef_method[2,],
                                            beta3 = coef_method[3,],
